...
class Solution:
    ...
    def get_list_lengths(self, total_length, k):
        sub_list_len = total_length//k
        remainder = total_length%k
        list_lengths = [sub_list_len for _ in range(k)]
        sub_list_idx = 0
        while remainder:
            list_lengths[sub_list_idx] += 1
            sub_list_idx += 1
            remainder -= 1
        return list_lengths
    ...
    def splitListToParts(self, root, k):
        ...
        if root:
            total_length = self.list_len(root)
            list_lengths = self.get_list_lengths(total_length, k)
            prev = None
            curr = root
            count = 0
            list_idx = 0
            while curr:
                if count < list_lengths[list_idx]:
                    if count == 0:
                        if prev:
                            prev.next = None
                        result[list_idx] = curr
                    prev = curr
                    curr = curr.next
                    count += 1
                else:
