...
class MyCalendar {
    ...
    std::list<Interval> calendar;
    bool overlap(const Interval& lhs, const Interval& rhs)
    {
        if (rhs.start <= lhs.start && lhs.start < rhs.end) return true;
        if (lhs.start <= rhs.start && rhs.start < lhs.end) return true;
        return false;
    }
public:
    MyCalendar() {}
    ...
    bool book(int start, int end)
    {
        ...
        calendar.sort(CompareInterval());
        for (const auto& e : calendar) {
            if (overlap(temp, e)) return false;
        }
        calendar.push_back(temp);
        return true;
    }
